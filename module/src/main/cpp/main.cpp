...
#include "Logger.h"
#include "MainLooper/MainLooper.h"
#include "Menu.h"

static int enable_hack;
static bool init_false = false;
...
int isGame(JNIEnv *env, jstring appDataDir) {
    ...
    if (strcmp(package_name, "com.tencent.tmgp.pubgmhd") == 0) {
...
static void forkAndSpecializePost(JNIEnv *env, jclass clazz, jint res) {
    ...
    if (res == 0) {
        // In app process
        //LOGD("forkAndSpecializePost, enable_hack: %d", enable_hack);
        // in app process
        //如果是该进程，并且没有找到该模块，开启线程去寻找(会调用多次)
        if (enable_hack && !init_false) {
            init_false = true;
