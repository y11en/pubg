...
/*************************** 绘制配置 ***********************************/
bool showHealth = true, showLine = false, showCrosshair = true, showDis = true, showInfo = true, showRadar = true, showName = true, showTeam = true, showWeapon = true;
...
void initNativeDraw() {
    ...
    ANativeWindow_acquire(g_NativeWindow);
    {
        ...
        ANativeWindow_setBuffersGeometry(g_NativeWindow, ScreenInfo.widthPixels,
                                         ScreenInfo.heightPixels, egl_format);//缩放
    ...
    ImGui_ImplAndroid_Init(nullptr, {(float) screenWidth / (float) ScreenInfo.widthPixels,
                                     (float) screenHeight / (float) ScreenInfo.heightPixels});
...
EGLBoolean new_eglSwapBuffers(EGLDisplay dpy, EGLSurface surface) {
    ...
    if (!g_Initialized) {
        ...
        ImGui_ImplAndroid_Init(nullptr, {(float) screenWidth / (float) glWidth,
...
void *thread_onDraw(void *args) {
    ...
    while (!g_NativeWindow) {
        if (isFakeRecorder) {
            ...
            jmethodID getSurfaceTextureMethod = env->GetMethodID(textrueViewClass,
                                                                 "getSurfaceTexture",
            ...
            jmethodID surfaceConstructor = env->GetMethodID(surfaceClass, "<init>",
            ...
            jmethodID getHolderMethod = env->GetMethodID(surfaceViewClass, "getHolder",
            ...
            jmethodID getSurfaceMethod = env->GetMethodID(surfaceHolderClass, "getSurface",
...
void *hook_read_thread(void *) {
    ...
    //LOGD("hook_read_thread------------初始化g_App");
...
void *call_syscall(void *args) {
    ...
    void *ret = (void *) syscall((long) d_args[0], d_args[1], d_args[2], d_args[3], d_args[4],
                                 d_args[5], d_args[6]);
    return ret;
}
