...
static bool isSafeAddr(Vector3A point) {
    if (isnan(point.X) || isinf(point.X) || isnan(point.Y) || isinf(point.Y) || isnan(point.Z) ||
        isinf(point.Z)) {
        return true;
    } else {
        return false;
...
bool getisDie(int state, float health) {
    return (state == 1048576 || state == 0 || state == 1048577 || state == 1048592 ||
            state == 71894088 || health == 0);
}
...
static Tracking bulletTrack(Vector3A myLoc, bool isCusimg) {
    ...
    if (isCusimg) {
        ...
        memoryRead(
                playerCameraManagerBase + Offset::dw_CameraCacheEntry + Offset::dw_MinimalViewInfo +
                Offset::dw_Location, &myLocation, sizeof(myLocation));
    }
    ...
    if (aimAnticipation2 > 0) {
        ...
        //距离
        double distance = sqrtf(
                powf(aimObjInfo.X - myLocation.X, 2.0f) + powf(aimObjInfo.Y - myLocation.Y, 2.0f) +
                powf(aimObjInfo.Z - myLocation.Z, 2.0f));
        //子弹速度
        double aimoSpeed = getF(myGunBase + Offset::bulletVelocityOffset) * 0.01f;
        //到达耗时
        double flyTime = (distance / 100.0f) / aimoSpeed; //秒
        ...
        FRotator TargetRot = ToRotator({myLocation.X, myLocation.Y, myLocation.Z},
                                       {yupanPoint.X, yupanPoint.Y, yupanPoint.Z});
        trackData.aim_angle = {TargetRot.Pitch, TargetRot.Yaw, 0};
    } else {
        trackData.loc = aimObjInfo;
        FRotator TargetRot = ToRotator({myLocation.X, myLocation.Y, myLocation.Z},
                                       {aimObjInfo.X, aimObjInfo.Y, aimObjInfo.Z});
        trackData.aim_angle = {TargetRot.Pitch, TargetRot.Yaw, 0};
    }
    return trackData;
}
...
//            *(uintptr_t *) &AddControllerPitchInput = libUE4 + Offset::AddPitchInputOffset;
        ...
        //LOGD("读取一些基础数据 世界资源列表PersistentLevel: %lx", (unsigned long) PersistentLevel);
        ...
        //LOGD("读取一些基础数据 playerCameraManagerBase: %lx",(unsigned long) playerCameraManagerBase);
        //LevelActorContainer
//        uintptr_t LevelToLevelActorContainer = getA(PersistentLevel + Offset::ULevelToLevelActorContainer);
//        LOGD("读取一些基础数据 LevelToLevelActorContainer: %lx",(unsigned long) LevelToLevelActorContainer);
        //人物对象
        uintptr_t ActorAddress = getA(PersistentLevel + Offset::ULevelToAActors);
        //数组指针地址
        //uintptr_t ActorAddress = getA(LevelToLevelActorContainer + Offset::ULevelActorContainerToAActors);
        //LOGD("读取一些基础数据 ActorAddress: %lx",(unsigned long)ActorAddress);
        // int ActorSize= getI(ActorAddress);
        //LOGD("读取一些基础数据 ActorSize: %d",ActorSize);


        //数组大小
        int ResNum = getI(PersistentLevel + Offset::ULevelToAActorsCount);
        //数组大小
        // int ResNum = getI(LevelToLevelActorContainer + Offset::ULevelActorContainerToAActorsCount);

        actorsCount = 0;
        memset(update, 0, (MaxActorCount + 5) * sizeof(Update));
        //LOGD("读取一些基础数据 ResNum: %d",ResNum);
        //读取一些基础数据
        for (int i = 0; i < ResNum; i++) {
            ...
            if (isContain(ClassPath, "STExtraPlayerCharacter")) {
                ...
                if (isContain(ClassPath, "WerewolfBase")) {
                    ...
                    my_strcpy(update[actorsCount].ClassPath,
                              "STExtraPlayerCharacter.PlayerPawn_Hider");
                } else {
                ...
                if (!update[actorsCount].isAI && !update[actorsCount].isRat) {
                    ...
                    if (getA(NamePointer) <= 0x10000000) {
                        continue;
                    }
                    getPlayerName(NamePointer, update[actorsCount].PlayerName);

                    //LOGD("正常模式 PlayerName: %s", update[actorsCount].PlayerName);
                ...
                actorsCount++;
                //LOGD("读取一些基础数据 ObjectPointer: %lx", (unsigned long) ObjectPointer);
                //LOGD("读取一些基础数据 玩家添加 actorsCount:  %d", actorsCount);
            }
...
void createDataList() {
    ...
    // 是否在游戏大厅
    NetDriver = getA(uWorldAddress + Offset::UWorldToNetDriver);
    //LOGD("是否在游戏大厅 %lx ",(unsigned long)NetDriver);
    if (NetDriver == 0) {
        usleep(100);
        return;
    }
    ...
    playerCameraManagerBase = getA(playerController + Offset::playerCameraManagerOffset);
    //LOGD("createDataList playerCameraManagerBase");
    //自身阵营
...
//    //用于自瞄对象
    ...
    for (int i = 0; i < updateActorsCount; i++) {
        ...
        //LOGD("ObjectPointer--------------: %lx", (unsigned long)ObjectPointer);

        // 坐标指针(控制器)
        uintptr_t RootCompont = update_data[i].RootCompont;
        if (RootCompont == 0)
            continue;
        //LOGD("createDataList RootCompont--------------: %lx", (unsigned long)RootCompont);

        //检查更新后的正常数据是否再次被缺页
        if (!isSafeAddress(RootCompont, Offset::PointerSize))
            continue;
//        LOGD("update_data[i].ClassPath--------------3: %s",
//             update_data[i].ClassPath);
...
//            // 人物状态
            ...
            data.RotationAngle = rotateCoord(myyaw - 90.0f,
                                             (SelfInfo.X - ObjInfo.X) / 225,
                                             (SelfInfo.Y - ObjInfo.Y) / 225);
            // 雷达坐标
...
//
            ...
            PlayerCount++;
            //LOGD("PlayerCount: %d", PlayerCount);
            //判断180°FOV 自瞄
...
//        }
    ...
    //LOGD("createDataList updateActorsCount--------------4");
    DrawNum(PlayerCount);
//
//    //读取完数据挂起状态,等待被拉起
